...
def test_current_catalog_resume_state_advances_through_batch24_window():
    state = cb.compute_resume_state()

    assert state["clean_count"] == 438
    assert state["review_entries_count"] == 0
    assert state["review_overlap_count"] == 0
    assert state["review_only_blocked_count"] == 0
    assert state["active_blocked_count"] == 0
    assert state["blocked"] == 0
    assert state["resume_after_key"] == "IL-confirmed|Jeep|Avenger"
    assert state["next_key_to_process"] == "IL-likely|Jeep|Avenger"
    assert state["next_key"] == "IL-likely|Jeep|Avenger"
    assert state["next_make"] == "Jeep"
    assert state["next_model"] == "Avenger"
